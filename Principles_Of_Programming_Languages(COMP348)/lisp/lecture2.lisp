...
; global variable
(defparameter place 'canada)
(print place)
(defparameter place 'montreal) ; change value of global value
(print place)

; eql vs equal
(defparameter X '(A B C D E))
(defparameter Y '(A B C D E))
(defparameter Z *)
(print (eql x y)) ; exact same variable reference
(print (equal x y)) ; same value
(print (eql x z))
(print (equal x z))
...
(defun max2 (lst)
  (cond 
    ...
    (t (let ((a (car lst)) (b (max2 (cdr lst))))
      (if (> a b) a b)))))
